...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        onehot_out = []
        eps = 1e-8
        eps = th.tensor(eps).to(self.args.device)
        ...
        for t in range(batch.max_seq_length):
            ...
            att = att.to(self.args.device)
            onehot_out.append(F.kl_div((att+eps).log(), eye, reduction='none').mean(dim=-1))
            mac_out.append(agent_outs)
        mac_out = th.stack(mac_out, dim=1).to(self.args.device)  # Concat over time
        onehot_out = th.stack(onehot_out, dim=1).to(self.args.device)
